...
def get_assignment_input_schema():
    demands = get_demands_from_order_data_frame()
    trucks = create_truck_from_data_frame()
    
    ealiest_date = min(d.available_time.date() for d in demands)
    latest_date = max(d.due_time.date() for d in demands)
    planning_horizon = [ealiest_date + timedelta(days=i) for i in range((latest_date - ealiest_date).days + 1)]
